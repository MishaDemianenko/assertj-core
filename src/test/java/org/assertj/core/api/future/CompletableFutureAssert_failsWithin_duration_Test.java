...
@DisplayName("CompletableFutureAssert failsWithin(Duration)")
class CompletableFutureAssert_failsWithin_duration_Test {
  ...
  void should_allow_assertion_on_future_result_when_failed() {
    ...
    // WHEN/THEN
    assertThat(future).failsWithin(Duration.of(1, MILLIS))
  ...
  void should_fail_when_completable_future_is_null() {
    ...
    // WHEN
    AssertionError assertionError = expectAssertionError(() -> assertThat(future).failsWithin(Duration.of(1L, MILLIS)));
    // THEN
  ...
  void should_fail_if_completable_future_is_completed_exceptionally() {
    ...
    // WHEN
    AbstractThrowableAssert<?, ? extends Throwable> throwableAssert = assertThat(future).failsWithin(Duration.of(1L, MILLIS));
    // THEN
