...
public abstract class AbstractCompletableFutureAssert<SELF extends AbstractCompletableFutureAssert<SELF, RESULT>, RESULT> extends
  ...
  /**
   * Waits if necessary for at most the given time for this future to fail, and then returns exception for further assertions.
   * <p>
   * If the future does not throw exception for any reason an assertion error is thrown.
   * <p>
   * To get specific assertions for the future exception type use {@link #failsWithin(Duration, InstanceOfAssertFactory)} instead.
   * <p>
   * Examples:
   * <pre><code class='java'> CompletableFuture&lt;String&gt; future = CompletableFuture.runAsync(() -> {
   ...
   * @throws AssertionError if the actual {@code CompletableFuture} does not fails within the given timeout.
   */
   ...
   * If the future does not throw exception for any reason an assertion error is thrown.
   * <p>
   * To get specific assertions for the future exceptions type use {@link #failsWithin(long, TimeUnit, InstanceOfAssertFactory)} instead.
   * <p>
   * Examples:
   * <pre><code class='java'> CompletableFuture&lt;String&gt; future = CompletableFuture.runAsync(() -> {
   *       throw new RuntimeException("Exception");
   ...
   * CompletableFuture future = new CompletableFuture();
   * future.cancel(false);
   * assertThat(future).failsWithin(100, TimeUnit.MILLISECONDS);</code></pre>
   *
   * @param timeout the maximum time to wait
   ...
   * @return a new narrowed {@link Assert} instance for assertions chaining on the exceptions from the {@link CompletableFuture}
   * @throws AssertionError if the actual {@code CompletableFuture} is {@code null}.
   * @throws IllegalStateException if the actual {@code CompletableFuture} does not fails within the given timeout.
   */
  public AbstractThrowableAssert<?, ? extends Throwable> failsWithin(Duration timeout,
   ...
   * // assertion succeeds
   * // using asInstanceOf is recommended to get assertions for the future result's type
   * assertThat(future).failsWithin(100, TimeUnit.MILLISECONDS, InstanceOfAssertFactories.THROWABLE)
   *                   .hasMessage("Exception");
   *
   * @param timeout the maximum time to wait
   * @param unit the time unit of the timeout argument
